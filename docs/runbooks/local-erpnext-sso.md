...
| Newly provisioned site lacks `crm` or `workos_frappe_integration` | The stack is running the stock `frappe/erpnext` image or an older custom image. Check `docker compose -f ../infra/erpnext/pwd.yml exec -T backend ls apps`. |
| `bench --site <site> install-app workos_frappe_integration` fails `App workos_frappe_integration not in apps.txt` | `sites/apps.txt` (in the shared `sites` volume) predates the image swap. Regenerate: `docker compose -f ../infra/erpnext/pwd.yml exec -T backend bash -c 'ls -1 apps > sites/apps.txt'`. Locally the `configurator` service usually fixes this itself on `up`; on the production VM it does not (see `cloud-deploy.md`). |
...
## CRM↔ERPNext commercial hand-off (verified 2026-08-06)

Fresh tenants now activate this as part of provisioning. For an already-ready site, a System
Manager must enable both settings before testing a CRM Deal → Customer/Quotation flow:

- ERPNext Desk: `CRM Settings` → **Enable Frappe CRM Data Synchronization**.
- Frappe CRM Desk: `ERPNext CRM Settings` → enable the same-site integration, select the
  ERPNext Company, keep product synchronization enabled, and configure `Won` as the status
  that creates a Customer.

The settings are distinct doctypes despite their similar names. Confirm that the `Create
Quotation from CRM Deal` form script exists, then create an ERPNext Item with an Item Price.
The Cybranex-owned `workos_frappe_integration` app refreshes the linked CRM Product's displayed
rate when that Item Price changes. After rolling out an image containing this app to an existing
stack, run `bench --site <site> install-app workos_frappe_integration`, then
`bench --site <site> clear-cache`: Frappe caches the `doc_events` registry, so a container
restart alone does not expose a new hook to an already-running site.
