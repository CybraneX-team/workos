...
Tenant sites run `erpnext` **and** `crm` (Frappe CRM). Frappe apps live in the
image layer, not the `sites` volume, so both apps are baked into a custom image
rather than installed at runtime. Build inputs are version-controlled in
`infra/erpnext-image/` (`apps.json` pins the app branches; `Containerfile`
derives from `frappe/build` + `frappe/erpnext`).

```bash
az acr build --registry startupdigitaltwin123 \
  --image erpnext-crm:v16.26.1-crm1 --image erpnext-crm:latest \
...
#    for each existing site: docker compose -f pwd.yml exec -T backend bench --site <site> install-app crm
```
...
`install-app crm` fails with `App crm not in apps.txt`. Locally the `configurator`
