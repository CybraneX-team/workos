...
export async function setMetaConversionAction(companyId: string, actionType: string) {
  ...
  try {
    ...
    const metadata = { ...(rows[0].metadata ?? {}), meta_conversion_action_type: action };
    await client.query(
      `UPDATE public.integration_connections SET metadata = $2::jsonb
        WHERE company_id = $1 AND integration_id = 'int-meta'`,
      [companyId, JSON.stringify(metadata)],
    );
    await client.query(
      `UPDATE public.metric_sources SET status = 'needs_configuration'
        WHERE company_id = $1 AND integration_id = 'int-meta'
  ...
  return syncMetaCanonicalMetrics(companyId);
}
...
export async function configureMetaMetric(companyId: string, userId: string, key: MetaMetricKey, input: {
  ...
  const synced = await syncMetaCanonicalMetrics(companyId);
