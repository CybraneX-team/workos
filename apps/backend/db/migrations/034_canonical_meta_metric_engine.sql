-- Canonical metric engine cleanup + latest-only Meta integration support.
...
-- Formula/import sources were exposed before they had a working ingestion or
-- dependency-recomputation path. Remove those unfinished records explicitly.
DELETE FROM public.metrics m
WHERE EXISTS (
  SELECT 1 FROM public.metric_sources s
  WHERE s.metric_id = m.id
    AND s.source_type::text IN ('formula', 'imported_file')
);
...
ALTER TABLE public.metric_sources
  DROP CONSTRAINT IF EXISTS metric_sources_status_check;
ALTER TABLE public.metric_sources
  ADD CONSTRAINT metric_sources_status_check
  CHECK (status IN ('active', 'disconnected', 'needs_configuration'));
ALTER TABLE public.metric_sources
  DROP CONSTRAINT IF EXISTS metric_sources_supported_type_check;
ALTER TABLE public.metric_sources
  ADD CONSTRAINT metric_sources_supported_type_check
...
ALTER TABLE public.metric_values
  DROP CONSTRAINT IF EXISTS metric_values_supported_type_check;
ALTER TABLE public.metric_values
  ADD CONSTRAINT metric_values_supported_type_check
  CHECK (source_type::text IN ('manual', 'integration'));

-- Integration metrics retain only their latest canonical value. Keep the
-- newest pre-existing row before enforcing that invariant.
DELETE FROM public.metric_values older
USING public.metric_values newer
...
CREATE UNIQUE INDEX IF NOT EXISTS metric_values_latest_integration_unique
  ...
  WHERE source_type = 'integration'::public.metric_source_type;
