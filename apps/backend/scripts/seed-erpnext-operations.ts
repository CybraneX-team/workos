...
async function main() {
  ...
  const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'erpnext_operations_seed.py');
  const containerPath = '/home/frappe/frappe-bench/apps/frappe/frappe/operations_seed.py';
  ...
  const { stdout, stderr } = await execFileAsync(
    'docker',
    ['compose', '-f', 'pwd.yml', 'exec', '-T', 'backend', 'bench', '--site', creds.siteName, 'execute', 'frappe.operations_seed.seed'],
    { cwd: env.FRAPPE_DOCKER_DIR, timeout: 180_000, maxBuffer: 10 * 1024 * 1024 },
  );
